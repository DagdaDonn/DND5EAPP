...
REM from. (The spec file itself -- packaging\windows\DnD5eCharacterCreator.spec --
...
REM the repo root. This script lives two levels down, in
REM installer\windows\, so it takes two ".." to reach the repo root.)
cd /d "%~dp0..\.."
...
REM build; pass "clean" as an argument (installer\windows\build_exe.bat clean) when you
...
if defined PYI_CLEAN_FLAG (
    ...
    echo   Run "installer\windows\build_exe.bat clean" to force a full rebuild.
)
echo.
python -m PyInstaller %PYI_CLEAN_FLAG% --noconfirm --distpath dist --workpath build packaging\windows\DnD5eCharacterCreator.spec
if %errorlevel% neq 0 (
    echo.
    echo ERROR: Build failed. Common causes:
    echo   - Missing icon file: make sure dnd_app\ui_desktop\icon.ico exists
