...
# (The spec file itself -- packaging/windows/DnD5eCharacterCreator.spec --
# resolves its own paths via SPECPATH, so it doesn't actually depend on
# this cd; but --distpath/--workpath below are plain CLI args resolved
# against CWD, so this still matters for dist/ and build/ landing at the
# repo root. This script lives two levels down, in installer/windows/, so
# it takes two ".." to reach the repo root.)
...
if [ "$1" = "clean" ]; then
    ...
    echo "Run './installer/windows/build_exe.sh clean' to force a full rebuild."
fi
python3 -m PyInstaller $CLEAN_FLAG --distpath dist --workpath build packaging/windows/DnD5eCharacterCreator.spec
