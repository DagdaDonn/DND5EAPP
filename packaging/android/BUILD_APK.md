...
8. **Create `deployment/recipes/p4a_hook.py`.** This is a build hook
   this project needs that isn't generated by anything. `deployment/`
   is gitignored (it's mostly a multi-GB build cache), but this one
   file is a tracked exception (see the `.gitignore` comment above
   `!deployment/recipes/p4a_hook.py`) since it's hand-written and
   load-bearing: without it, Qt refuses to load at runtime (missing
   `libc++_shared.so`) *and* the Java compile fails (Qt 6.11 removed
   the method p4a `v2024.01.21`'s template still calls). If it's
   already committed, you don't need to write it from scratch — just
   confirm it's present. See "Why `p4a.hook` exists" below for exactly
   what it does and does not do — do not skip this step or improvise a
   different version without understanding both
...
(tracked despite living under gitignored `deployment/` — see step 8
above). p4a runs this at two points during the build and it fixes two
separate, otherwise-fatal problems with p4a `v2024.01.21`'s Qt
bootstrap template:

- **`before_apk_build`** does two things. First, it copies
  `libc++_shared.so` from the NDK into the dist's `libs/arm64-v8a/`
  (and removes a stale copy under `src/main/jniLibs/arm64-v8a/`) —
  without it, Qt refuses to load and the app crashes immediately on
  launch, and since the *build* still succeeds, this is easy to
  misdiagnose as an app-code bug instead of a packaging one. Second, it
  flattens every native runtime `.so` the PySide6/shiboken6 bundle
  needs — `libQt6*.so` from `PySide6/Qt/lib/`, Qt's own plugins
  (`PySide6/Qt/plugins/`), every QML module's plugin `.so`
  (`PySide6/Qt/qml/`), and even the top-level `Qt*.abi3.so`/
  `Shiboken.abi3.so` files — into `libs/arm64-v8a/`. That last part is
  counterintuitive: an `.abi3.so` suffix normally means "Python
  extension module," but Qt 6.11's Android bootstrap calls
  `System.loadLibrary("QtQuick")` etc. during `onCreate`, and Android's
  loader needs to find those `.abi3.so` files as if they were native
  libraries, not just importable Python modules.
- **`before_apk_assemble`** patches the dist's generated
  `PythonActivity.java` three ways: replaces
  ...
  still calls it — without this, the build fails at the Java compile
  step, before an APK is even produced); strips a stale reflective
  `QtNative.startApplication(...)` call from an earlier, wrong attempt
  at this same fix (`QtActivityBase` already calls `startApplication`
  internally — that reflection call was never needed); and **prepends
  an explicit `System.loadLibrary(...)` preload loop, in dependency
  order, for the whole Qt Quick chain** (`Qt6Core` first, then
  `Network`, `Gui`, `OpenGL`, `Qml` and its sub-modules, then the
  QuickControls2/Layouts/Effects/Shapes/VectorImage/Dialogs stack,
  `Qt6Quick` last) to the top of `PythonActivity.onCreate`. **Confirmed
  fixed, not a hypothesis:** without this, Android's loader falls back
  to loading `libQt6Quick` first; `Qt6Core`'s own `JNI_OnLoad` (which
  caches the process's `JavaVM*` for `QJniEnvironment::getJniEnv()` to
  find later) never gets the chance to run before something on the
  `qtMainLoopThread` needs it, and the app segfaults instantly
  (`SIGSEGV` at `fault addr 0x0`) the moment it does. Loading every
  library up front, main thread, in the right order, means every
  `JNI_OnLoad` runs with the JVM already cached.

Two more pieces of the real, confirmed build config, both in
`buildozer.spec` (not the hook), that this same failure mode taught us
are load-bearing:

- **`p4a.extra_args` includes `--private=/tmp/mimic-app-staging`** — a
  real, populated staging directory p4a copies the app's actual Python
  source from, rather than trusting `source.include_exts`/
  `source.include_patterns` alone to have assembled the right tree.
  That staging directory must contain `main.py` and all of `dnd_app/`
  — **`ui_desktop/` included**, since several `ui_android` bridges
  (e.g. `character_sheet.py`) import pure-logic pieces of it directly
  (theme colors, flavor text, level-up choice tables) rather than
  duplicating that logic — see `buildozer.spec`'s own
  `source.include_patterns` comment for the exact reasoning. Don't
  "clean up" by excluding `ui_desktop/` from the Android build; it will
  break with `ModuleNotFoundError: No module named 'dnd_app.ui_desktop'`.
- **A `blacklist.txt` inside the dist should include `lib2to3/tests/*`**
  — without it, `compileall` can die trying to compile Python 2 test
  fixtures that ship inside CPython's own stdlib. This lives inside the
  generated dist, not this repo, so it needs re-applying if the dist is
...
one of the failures above, and each surfaces only after the slow
20-40 minute path since every hook point runs late in the build.
...
| `SIGSEGV` at `fault addr 0x0` on the `qtMainLoopThrea` thread, tombstone shows `QJniEnvironment::getJniEnv()` → `libQt6Quick*.so`'s `JNI_OnLoad` | A Qt library other than `Qt6Core` had its `JNI_OnLoad` run first (no cached `JavaVM*` yet) — see the third `p4a_hook.py` bullet under "Why `p4a.hook` exists" above |
...
| Java compile fails with "package org.qtproject.qt.android.bindings does not exist" | `android.add_jars` is missing, wrong, or has a duplicate entry — check it points at `Qt6Android.jar`, `Qt6AndroidBindings.jar`, and `Qt6AndroidQuick.jar`, each listed once |
| App loads, `libQt6Quick_arm64-v8a.so` fails with `JNI_ERR returned from JNI_OnLoad` (build compiled fine, this is runtime-only) | `android.add_jars` is missing `Qt6AndroidQuick.jar` — Qt6Quick's own `JNI_OnLoad` registers native methods against `org.qtproject.qt.android.QtQuickView`, which lives in that jar (confirmed against `qtdeclarative`'s real 6.11.2 source, `src/quick/platform/android/jar/`); without it, `registerNatives()` fails and `JNI_OnLoad` returns `JNI_ERR`, aborting QtLoader's whole load sequence |
| `ModuleNotFoundError` for `dnd_app.core`/`dnd_app.data`/`dnd_app.ui_desktop` at runtime on-device | Check `buildozer.spec`'s `source.include_patterns`/`p4a.extra_args`'s `--private=/tmp/mimic-app-staging` staging dir actually contains that module — see "Why `p4a.hook` exists" above for why `ui_desktop` specifically must be included |
| `compileall`/build fails compiling something under `lib2to3/tests/` | The dist's `blacklist.txt` needs `lib2to3/tests/*` added — those are Python 2 test fixtures bundled with CPython's stdlib that don't need compiling |
| Save/Export doesn't behave as expected on-device | See the storage-permissions note in `packaging/android/README.md` |
| App loads then dies silently / classes or reflection calls that used to work stop working, after a build that previously worked | Check `buildozer.spec` for `android.debug`, `android.release_artifact`, or `p4a.build_mode = release` — a release build task can enable R8/ProGuard minification, which strips/renames classes by default and silently breaks `p4a_hook.py`'s `Class.forName("org.qtproject.qt.android.QtNative")` reflection call without a keep rule. `packaging/android/snapshots/working-20260917-1224/` is a known-good debug-only config to diff against when something regresses unexplainably |
