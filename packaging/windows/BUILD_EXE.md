...
`installer\windows\` also works.) The script handles everything. The finished EXE
...
`installer\windows\build_exe.bat` / `./installer/windows/build_exe.sh` no longer force
...
directory from scratch every time. Run `installer\windows\build_exe.bat clean` /
`./installer/windows/build_exe.sh clean` to force a full rebuild — do this after
